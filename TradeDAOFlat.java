...
//	public static final String userdirectory = ".";
	public static final String userdirectory = "/mapr/mcade.cluster.com/user/ec2-user";
	...
	public List<Trade> getTradesByDate(String symbol, Long from, Long to)
		...
		for (Result result : scanner) { // scanner has one row result per Symbol
			...
			for (byte[] hour : hours) {
				List<Cell> priceKVs = result.getColumnCells(priceCF, hour);
	...
	@Override
	public Trade getTrade(String symbol, Long tradeDt) throws IOException {
		Get g = new Get(Bytes.toBytes(formRowkey(symbol, tradeDt)));
		g.addFamily(priceCF);
		g.addFamily(volumeCF);

		Result result = table.get(g);
		if (result.isEmpty()) {
			System.out.println(String.format("Trade for %s and date %d not found.", symbol, tradeDt));
			return null;
		}
		...
		for (byte[] hour : hours) {
			...
			for (int i = 0; i < priceKVs.size(); i++) {
				...
				// Add the new trade to the list of trades
				//trades.add(new Trade(symbol, price, volume, time));
			    System.out.println(symbol + " " + price + " " + volume + " " + time);
